...
export default function AssessmentArena() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const session = response.data.find((s: any) => s.id === Number(sessionId));
        if (session) {
          setSessionData(session);
          // Normally session metrics would give us the problem list
          const probs = session.metrics?.problems || [];
          if (probs.length > 0) {
            setActiveProblemId(probs[0].id);
          }
        }
      } catch (err) {
        console.error(err);
  ...
  if (!sessionData) return <div className="p-8 text-white">Loading Arena...</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 overflow-hidden">
        ...
        <div className="flex flex-col space-y-4 overflow-hidden">
          <Card className="p-4 bg-[#0F172A] border-[#1F2937] overflow-y-auto">
            <h3 className="text-lg font-semibold text-white mb-2">Problem List</h3>
            <div className="flex flex-wrap gap-2 mb-4">
              {sessionData.metrics?.problems?.map((p: any, idx: number) => (
                <button
                  key={p.id}
        ...
        <div className="flex flex-col space-y-4">
          <Card className="flex-1 bg-[#1E1E1E] border-[#333] flex flex-col overflow-hidden relative">
